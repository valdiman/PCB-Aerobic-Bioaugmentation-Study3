# Code to model PCB 1 in laboratory experiments
# using sediment from NBH (site INT 222). Passive measurements
# of PCB 1 in the water and the air phases are predicted and
...
rtm.PCB1 = function(t, state, parms){
  ...
  Kaw <- 0.01217 # PCB i dimensionless Henry's law constant @ 25 C
  dUaw <- 47.73474 # internal energy for the transfer of air-water for PCB i (J/mol)
  Kaw.t <- Kaw * exp(-dUaw / R * (1 / Tw.1 - 1 / Tst.1)) * Tw.1 / Tst.1
  Kow <- 10^(4.46) # PCB i octanol-water equilibrium partition coefficient
  dUow <-  -21.689 # internal energy for the transfer of octanol-water for PCB i (J/mol)
  Kow.t <- Kow * exp(-dUow / R * (1 / Tw.1 -  1/ Tst.1))
  Koa <- 10^(6.374555) # PCB i octanol-air equilibrium partition coefficient
  
  # PUF constants 
  Apuf <- 7.07 # cm2
  Vpuf <- 29 # cm3 volume of PUF
  d <- 0.0213 * 100^3 # g/m3 density of PUF
  Kpuf <- 10^(0.6366 * log10(Koa) - 3.1774) # PCB i-PUF equilibrium partition coefficient [m3/g]
  Kpuf <- Kpuf * d # [La/Lpuf]
  ...
  Kf <- 10^(1.06 * log10(Kow.t) - 1.16) # PCB i-SPME equilibrium partition coefficient
  ...
  D.pcb.air <- D.water.air * (MW.pcb/MH2O)^(-0.5) # cm2/s PCB i's diffusion coefficient in the gas phase (eq. 18-45)
  D.pcb.water <- D.co2.w * (MW.pcb/MCO2)^(-0.5) # cm2/s PCB i's diffusion coefficient in water @ Tair = 25 C, patm = 1013.25 mbars
  ...
  SC.pcb.w <- v.H2O / D.pcb.water # Schmidt number PCB i
  ...
  # ii) Kaw.w, water-side mass transfer coefficient for PCB i. 600 is the Schmidt number of CO2 at 298 K
  Kaw.w <- V.co2.w * (SC.pcb.w/600)^(-0.5) # [m/s]
  # iii) kaw, overall air-water mass transfer coefficient for PCB i
  kaw.o <- (1 / (Kaw.a * Kaw.t) + (1 / Kaw.w))^-1 # [m/s]
  # iv) kaw, overall air-water mass transfer coefficient for PCB i, units change
...
{
  Ct <- 5 # ng/g PCB i sediment concentration av. of site INT 222 (10.8, stdv = 4 ng/g)
  n <- 0.42 # [%] porosity
  ds <- 1540 # [g/L] sediment density
  M <- ds * (1 - n) / n # [g/L]
  Cs0 <- Ct * M # [ng/L]
...
parms <- list(ro = 2000, ko = 10, kb = 0) # Input 500/540 non-shaking/shaking
t.1 <- unique(pcb_combined_control$time)
# Run the ODE function without specifying parms
out.1 <- ode(y = cinit, times = t.1, func = rtm.PCB1, parms = parms)
head(out.1)

{
  ...
  out_daily <- ode(y = cinit, times = t_daily, func = rtm.PCB1,
                   parms = parms)
  head(out_daily)
  ...
  # Export data
  write.csv(model_results_daily_clean, file = "Output/Data/RTM/PCB1Control.csv")
...
p.1 <- grid.arrange(p_mf, p_mpuf, ncol = 2)

# Save plot in folder
ggsave("Output/Plots/RTM/PCB1Control.png", plot = p.1, width = 6,
       height = 5, dpi = 500)
