...
rtm.PCB31 = function(time, state, parms){
  ...
  # ii) Kaw.w, water-side mass transfer coefficient for PCB 31. 600 is the Schmidt number of CO2 at 298 K
  Kaw.w <- V.co2.w * (SC.pcb.w/600)^(-0.5) # [m/s]
  # iii) kaw, overall air-water mass transfer coefficient for PCB 31
  kaw.o <- (1 / (Kaw.a * Kaw.t) + (1 / Kaw.w))^-1 # [m/s]
  # iv) kaw, overall air-water mass transfer coefficient for PCB 31, units change
  ...
  # Time-dependent fraction of biochar-sorbed PCB accessible to SPME
  alpha_max <- parms$alpha
  k_alpha   <- parms$k_alpha
  t50       <- parms$t50
  
  alpha_t <- ifelse(
    time < t50,
    0,
    alpha_max * (1 - exp(-k_alpha * (time - t50)))
  )
  
  # Biochar-sorbed PCB near fiber
  Cbc_w <- Kbc * Cw * Cbc / 1000  # ng/L
  
  # Effective concentration seen by SPME
  Cw_eff <- Cw + alpha_t * Cbc_w
  ...
  dCfdt  <- ko * Af / Vf * (Cw_eff - Cf / Kf) # Cw = [ng/L], Cf = [ng/L]
...
{
  Ct <- 1803.478 # ng/g PCB 31 sediment concentration av. of site INT 222 (stdv = 555 ng/g)
  n <- 0.42 # [%] porosity
  ds <- 1540 # [g/L] sediment density
  M <- ds * (1 - n) / n # [g/L]
  Cs0 <- Ct * M # [ng/L]
...
parms <- list(ro = 600, ko = 3, kb = 0, alpha = 0.7, k_alpha = 0.15, t50 = 90) # Input 500/540 non-shaking/shaking
...

{
  ...
  out_daily <- ode(y = cinit, times = t_daily, func = rtm.PCB31,
                   parms = parms)
  head(out_daily)
  ...
  # Export data
  #write.csv(model_results_daily_clean, file = "Output/Data/RTM/PCB31Control5.csv")
...
p.31 <- grid.arrange(p_mf, p_mpuf, ncol = 2)

# Save plot in folder
ggsave("Output/Plots/RTM/PCB31Control5.png", plot = p.31, width = 6,
       height = 5, dpi = 500)
